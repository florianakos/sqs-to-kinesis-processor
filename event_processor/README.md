...
Another possibility would be to use a so-called **dead-letter-queue** where SQS messages would be sent if the submission is invalid. This can be achieved by setting the `maxReceiveCount` parameter of the min queue, which defines how many times the consumer can try to process a message before it should be redirected to the DLQ. Once the invalid submission is sent to the DLQ there may be some alerts that trigger an notification so someone can investigate. In case the amount of these invalid submissions is too high, it may be impractical to investigate them all, so alerting should be tuned appropriately.
...
This partially depends on how critical this application is.

Most likely, I would create a CI/CD build which would get triggered from VCS if some part of the code base changes. This CI build run the unittests to verify that the tests covering the processor functions still pass. 

I would also set up another CI/CD build that would act as an integration test or quality gate. This build would run a setup similar to how it's currently done in docker-compose. The **sensor-fleet** would need some tweaks so that it only submits a certain number of known messages. Then some tests could be written to connect to the Kinesis stream, read all the records and compare if they match the expectations based on the known submissions. 

Finally, using some IaC tool like Terraform, I would likely set up 3 identical environments in AWS, one for testing, one for staging and one for production. With Terraform it would be quite easy to define some environment based differences, such as number of shards in test, so that it's cheaper in testing where there may not be a need for high throughput. In prod there would probably be more shards and more replicas of this application. The runtime environment can either be some K8s or ECS cluster running a docker image built from the **event_processor** code base.
